...
void Graph::paintEvent( QPaintEvent * )
{
    ...
    paint.setPen(normalcolor);                      // do not draw outline
        ...
        if ((spectrumptr->isexcluded(index))&&(layer==1)) paint.setPen(excludecolor);
             ...
             if ((int)mycolors.size()>layer){
                ...
                if (stylelist[layer]==1){
                    //draw a line
                    if (((index+1)<data[layer].size())&&(index>=0)){
                    ...
                    //draw a circle on the datapoints
                     if (((index)<data[layer].size())&&(index>=0)){
